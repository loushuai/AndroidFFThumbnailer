...
int JpegWriter::init(AVFrame *frame)
{
	int ret = -1;
	...
	mCodecCtxPtr->pix_fmt    = AV_PIX_FMT_YUVJ420P;
	mCodecCtxPtr->codec_id   = AV_CODEC_ID_MJPEG;
...
int JpegWriter::writeFrame(AVFrame *frame, int quality)
{
	int ret = -1;
	int gotframe = 0;
